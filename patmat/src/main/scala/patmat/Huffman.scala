...
object Huffman {
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      def rec(tree: CodeTree, acc: List[Bit])(c: Char): List[Bit] = tree match {
        case Leaf(_, _) => acc
        case Fork(l, r, _, _) => if (chars(l) contains c) rec(l, 0 :: acc)(c) else rec(r, 1 :: acc)(c)
      }

      //text.foldLeft(List.empty[Bit])( (accum, element) => accum :: rec( element , tree, List.empty[Bit] ))

      text.flatMap( c =>  (rec(tree, List.empty[Bit])(c)).reverse)
    }
